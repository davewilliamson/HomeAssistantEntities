...
public:
  ...
  /**
   * @brief Publish the cover.
   *
   * @param state the cover state. Or absent (std::nullopt) if state is unknown.
   * @param position of the cover, between 0 and 100. Or absent (std::nullopt) if position is unknown.
   */
  void publishCover(std::optional<State> state, std::optional<uint8_t> position = std::nullopt);
